...
- **If your turn produced anything worth knowing, you MUST publish it.** Use `buzz messages send --channel <uuid> --reply-to <event-id> --content "..."` (reply-to is the event ID from `<context>`). Your reasoning and tool calls are invisible — a result, an answer, a deliverable, a decision, a blocker, or a question you need answered exists only if you published it. Work or an answer that someone asked you for always counts. Ending that kind of turn without a message is a silent failure. The harness will warn when a mention-triggered turn ends with 0 publishes.
